package pavel_sdk_test_python::pavel_sdk_test_pythonClient;
...
pavel_sdk_test_python::pavel_sdk_test_pythonClient
...
sub new
{
    ...
    my $self = {
	client => pavel_sdk_test_python::pavel_sdk_test_pythonClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$workspace_name is a pavel_sdk_test_python.workspace_name
$contigset_id is a pavel_sdk_test_python.contigset_id
$return is a pavel_sdk_test_python.CountContigsResults
...
# Authentication: required
	...
	method => "pavel_sdk_test_python.count_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
Person is a reference to a hash where the following keys are defined:
	...
	person_url has a value which is a string
...
# Authentication: required
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
	method => "pavel_sdk_test_python.get_person",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method get_person",
					    status_line => $self->{client}->status_line,
					    method_name => 'get_person',
				       );
    }
...
sub version {
    my ($self) = @_;
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "pavel_sdk_test_python.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(
            error => "Error invoking method get_person",
            status_line => $self->{client}->status_line,
            method_name => 'get_person',
        );
    }
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for pavel_sdk_test_python::pavel_sdk_test_pythonClient\n";
    }
    if ($sMajor == 0) {
        warn "pavel_sdk_test_python::pavel_sdk_test_pythonClient version is $svr_version. API subject to change.\n";
    }
}
...
package pavel_sdk_test_python::pavel_sdk_test_pythonClient::RpcClient;
